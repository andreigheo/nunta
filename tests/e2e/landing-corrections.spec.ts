...
const chapters = ["planificare", "invitatii", "furnizori", "ziua-evenimentului"];
...
test("navigarea publică descrie destinațiile reale", async ({ page }) => {
  ...
  for (const id of chapters) {
    ...
    await expect.poll(async () => page.locator(`#${id}`).evaluate(el => Math.round(el.getBoundingClientRect().top))).toBeGreaterThanOrEqual(72);
    await expect.poll(async () => page.locator(`#${id}`).evaluate(el => Math.round(el.getBoundingClientRect().top))).toBeLessThan(200);
  }
});
...
test("demonstrațiile nu oferă controale fără acțiune", async ({ page }) => {
  ...
  expect(await metricLinks.evaluateAll(links => links.every(link => link.getAttribute("href")?.startsWith("/produs#")))).toBe(true);
});

for (const width of [320, 360, 390, 768, 820]) {
  test(`mobil ${width}: conținut complet, FAQ și comparație fără glisare`, async ({ page }) => {
    ...
    for (const summary of await summaries.all()) await expect(summary).toBeVisible();
...
test("headerul rămâne lizibil la trecerea între dimensiuni", async ({ page }) => {
  for (const width of [820, 900, 1024, 1100, 1101, 1280, 1440]) {
    ...
    const sizes = await page.locator("header a").evaluateAll(links => links.filter(link => link.getBoundingClientRect().height).map(link => parseFloat(getComputedStyle(link).fontSize)));
  ...
  await expect(page.getByRole("navigation", { name: "Navigație mobilă" })).toBeHidden();
});
