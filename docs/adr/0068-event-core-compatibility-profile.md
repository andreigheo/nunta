...
The physical `wedding_profiles.wedding_date` storage and stable `couple_owner`
role key remain compatibility identifiers. Application code uses the
`EventProfile` Prisma model and `eventDate` field, mapped onto those existing
database identifiers without rewriting data. APIs return both `eventDate` and
the deprecated `weddingDate` alias. New clients must write `eventDate`; old
clients continue to work without a data rewrite or a breaking role migration.
...
2. Expose neutral role display names while preserving stable authorization
   keys.
3. Use `/event-day` as the canonical route for day-of-event operations. Keep
   `/wedding-day` as a deprecated compatibility alias until client telemetry
   shows no consumers.
4. Rename physical tables only in a separately rehearsed migration; this ADR
   does not authorize a destructive storage rename.
