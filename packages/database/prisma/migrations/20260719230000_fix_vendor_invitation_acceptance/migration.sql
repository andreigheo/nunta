...
BEGIN
  ...
  ON CONFLICT ON CONSTRAINT vendor_organization_memberships_vendor_organization_id_user_key
  DO UPDATE SET
    role_template_id = EXCLUDED.role_template_id,
    status = 'ACTIVE',
    joined_at = COALESCE(vendor_organization_memberships.joined_at, now()),
    removed_at = NULL,
