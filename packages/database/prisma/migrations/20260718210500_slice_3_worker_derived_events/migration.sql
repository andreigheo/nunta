-- Extend the persisted-context-only worker emission contract for Slice 3.
...
BEGIN
  ...
  IF derived_payload ? 'notification' THEN
    INSERT INTO public.outbox_consumer_executions (id, outbox_message_id, consumer_name, deduplication_key, updated_at)
    VALUES (gen_random_uuid(), derived_outbox_id, 'notification_projection', 'consumer:' || derived_outbox_id::text || ':notification_projection', now())
    ON CONFLICT (outbox_message_id, consumer_name) DO NOTHING;
  END IF;
  IF derived_payload ? 'activity' THEN
    INSERT INTO public.outbox_consumer_executions (id, outbox_message_id, consumer_name, deduplication_key, updated_at)
    VALUES (gen_random_uuid(), derived_outbox_id, 'activity_projection', 'consumer:' || derived_outbox_id::text || ':activity_projection', now())
    ON CONFLICT (outbox_message_id, consumer_name) DO NOTHING;
  END IF;
  IF derived_payload ? 'campaignDelivery' THEN
    INSERT INTO public.outbox_consumer_executions (id, outbox_message_id, consumer_name, deduplication_key, updated_at)
    VALUES (gen_random_uuid(), derived_outbox_id, 'campaign_delivery', 'consumer:' || derived_outbox_id::text || ':campaign_delivery', now())
    ON CONFLICT (outbox_message_id, consumer_name) DO NOTHING;
  END IF;
  IF derived_payload ? 'campaignSummary' THEN
    INSERT INTO public.outbox_consumer_executions (id, outbox_message_id, consumer_name, deduplication_key, updated_at)
    VALUES (gen_random_uuid(), derived_outbox_id, 'campaign_summary', 'consumer:' || derived_outbox_id::text || ':campaign_summary', now())
    ON CONFLICT (outbox_message_id, consumer_name) DO NOTHING;
  END IF;
