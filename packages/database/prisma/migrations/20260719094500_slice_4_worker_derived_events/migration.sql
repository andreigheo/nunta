-- Extend the persisted-context-only worker emission allow-list for the
-- deterministic seating suggestion. The function still derives tenancy from
-- the active OutboxConsumerExecution context and accepts no arbitrary worker
