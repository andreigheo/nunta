-- A worker may emit a narrowly-scoped semantic follow-up event only after its
-- persisted consumer execution proves workspace and actor ownership. The
-- function creates durable delivery intent and per-consumer executions in one
-- transaction; arbitrary BullMQ payload values never grant access.
...
DECLARE
  ...
  consumer text;
BEGIN
  IF derived_event_name NOT IN ('planning.plan_proposal_ready.v1', 'task.reminder_due.v1') THEN
