CREATE FUNCTION public.weddingos_get_weekly_digest_recipient(
...
BEGIN
  IF NOT public.weddingos_worker_execution_context_matches(
    ...
    RAISE EXCEPTION 'persisted weekly digest recipient context required' USING ERRCODE = '42501';
  END IF;
...
REVOKE ALL ON FUNCTION public.weddingos_get_weekly_digest_recipient(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.weddingos_get_weekly_digest_recipient(uuid, uuid) TO weddingos_worker;
