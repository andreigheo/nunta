...
const webServerCommand = process.env.WEDDINGOS_E2E_REUSE_BUILD
  ? "env NEXT_DIST_DIR=.next-e2e API_INTERNAL_URL=http://127.0.0.1:4117 NEXT_PUBLIC_DEMO_MODE_ENABLED=true corepack pnpm exec next start --hostname 127.0.0.1 --port 3117"
  : "env NEXT_DIST_DIR=.next-e2e API_INTERNAL_URL=http://127.0.0.1:4117 NEXT_PUBLIC_DEMO_MODE_ENABLED=true sh -c 'corepack pnpm exec next build --webpack && exec corepack pnpm exec next start --hostname 127.0.0.1 --port 3117'";
