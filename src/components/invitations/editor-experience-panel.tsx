    ...
    <div className="space-y-5 p-4">
      <div className="flex items-start justify-between gap-4">
        ...
        <Switch
          ...
          aria-label="Activează deschiderea cinematică"
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Monogramă">
          <Input
            ...
            maxLength={12}
            placeholder="A & M"
            onChange={(event) => onChange({ monogram: event.target.value || null })}
          />
        </Field>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Textură">
          <Select
            ...
            onChange={(event) =>
              onChange({
                texture: event.target.value as InvitationExperienceSettings["texture"],
              })
            }
          >
      ...
      <div className="space-y-2 rounded-xl bg-subtle/70 p-3 text-xs text-muted">
        <p className="flex gap-2">
          <Eye className="mt-0.5 size-3.5 shrink-0 text-brand" aria-hidden />
          Prima deschidere reală rulează revelația; revenirile intră direct în
          invitație și pot folosi „Revede introducerea”.
        </p>
        <p className="flex gap-2">
          <Gauge className="mt-0.5 size-3.5 shrink-0 text-success" aria-hidden />
          Pentru reduced motion, conținutul apare imediat, fără tranziții care
          blochează accesul.
        </p>
        <p className="flex gap-2">
          <Sparkles className="mt-0.5 size-3.5 shrink-0 text-accent" aria-hidden />
          Mișcarea folosește doar transformări accelerate și nu pornește audio.
        </p>
      </div>
    </div>
  );
