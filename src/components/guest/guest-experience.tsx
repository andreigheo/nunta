...
import type { GuestRsvpBootstrapResource } from "@weddingos/contracts";
...
export default function GuestRsvpPage() {
  ...
  const [data, setData] = React.useState<GuestRsvpBootstrapResource | null>(null);
  ...
  const submit = async () => {
    ...
    try {
      ...
      setData(await weddingOsApi.guestRsvpBootstrap(token));
    } catch (caught) {
      toast({
  ...
  const portal = (
    <PortalShell
      role="Confirmare RSVP"
      title={data ? `Confirmarea pentru ${data.household.name}` : "Confirmare RSVP"}
      subtitle="Completează răspunsul fiecărei persoane. Poți reveni la invitație fără să pierzi datele salvate."
      ...
      {loading ? (
        ...
        <>
          <nav aria-label="Navigare invitat" className="mb-6 flex flex-wrap gap-3 text-sm">
            ...
            <Link
              href={`/guest/companion?token=${encodeURIComponent(token)}`}
              className="font-semibold text-muted underline decoration-line-strong underline-offset-4 hover:text-ink"
            >
              Detaliile evenimentului
            </Link>
          </nav>
          <section id="confirmare-rsvp" className="mx-auto max-w-3xl scroll-mt-6 space-y-6">
            <Card>
              <CardHeader>
                <div>
...
export function GuestEventDayPanel({
  token,
  events,
}: {
  token: string;
...
export function GuestOperationsCards({ operations }: { operations: GuestOperations }) {
