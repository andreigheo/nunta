...
export function normalizePublicProductProof(
  ...
  const candidates: Array<[string, string, PublicProofMetric]> = [
    [
      "planning",
      "Planuri cu următoarea acțiune",
      proof.flow.planning.nextActionCoveragePercent,
    ],
    [
      "rsvp",
      "Răspunsuri RSVP",
      proof.flow.rsvpAndLogistics.rsvpResponseRatePercent,
    ],
    [
      "procurement",
      "Cereri ajunse la rezervare",
      proof.flow.procurementAndBudget.rfqToBookingWorkspaceRatePercent,
    ],
    [
      "wedding-day",
      "Momente din desfășurător finalizate",
      proof.flow.weddingDay.runOfShowCompletionRatePercent,
    ],
  ];
  const metrics = candidates.map(
    ([key, label, metric]): PublishedProofMetric => ({
      key,
      label,
      state: metric.state,
      value:
        metric.state === "published" && metric.value !== null
  ...
  return {
    state:
      proof.freshness === "stale" || ageMs > 30 * 60 * 1_000
        ? "stale"
        : "fresh",
