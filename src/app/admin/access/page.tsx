...
export default function AccessPage() {
  ...
  return <AdminShell title="Acces administrativ" subtitle="Roluri și granturi active în mediul curent. Acordarea și revocarea se fac din fișa utilizatorului, cu parolă, MFA și audit." actions={<div className="flex gap-2"><Link href="/admin/users" className="inline-flex min-h-9 items-center rounded-lg border border-line bg-surface px-3 text-sm font-semibold hover:bg-subtle">Gestionează utilizatori</Link><Button size="sm" variant="outline" loading={loading} onClick={() => void load()}><RefreshCw className="size-4" />Actualizează</Button></div>}>
