...
export default function BudgetPage() {
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "plan") await weddingOsApi.upsertBudget(currentWorkspace.id, budget.plan?.version ?? null, { name: form.name || "Bugetul nunții", targetTotalMinor: minor(form.amount), contingencyPercent: Number(form.contingency || 0), status: "ACTIVE" });
