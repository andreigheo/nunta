...
const shortcuts = [
  {
    ...
    description: "Importă, segmentează și urmărește RSVP-urile.",
    href: "/guests",
    icon: Users,
  },
  {
    title: "Plan de mese",
    description: "Așază invitații și verifică rapid capacitatea.",
    href: "/seating",
    icon: LayoutTemplate,
  },
  {
    title: "Comparație oferte",
    description: "Evaluează prețul și condițiile furnizorilor.",
    href: "/offers",
    icon: ReceiptText,
  },
  {
    title: "Contracte",
    description: "Urmărește semnăturile, termenele și avansurile.",
    href: "/contracts",
    icon: FileSignature,
  },
  {
    title: "Transport",
    description: "Grupează traseele și necesarul de locuri.",
    href: "/transport",
    icon: Route,
  },
];
...
export default function ToolsPage() {
  ...
  const [dashboard, setDashboard] = React.useState<PlanningDashboard | null>(
    null,
  );
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      if (demoMode) {
        setDashboard(null);
        setGuests(wedding.estimatedGuests);
        setBudget(wedding.targetBudget);
      } else {
        const value = await weddingOsApi.dashboard(currentWorkspace.id);
        setDashboard(value);
        setGuests(
          value.guestCrm.estimatedGuests ?? value.guestCrm.activeGuests,
        );
        setBudget(value.commercial.budget.targetTotalMinor / 100);
      }
    } catch (caught) {
  ...
  const copyDrinks = async () => {
    ...
    try {
      ...
      toast({
        title: "Estimare copiată",
        description:
          "O poți trimite acum locației sau furnizorului de băuturi.",
        variant: "success",
      });
    } catch {
      toast({
  ...
  if (error) {
    return (
      <ErrorState
        title="Uneltele nu și-au putut încărca datele"
        description={error}
        onRetry={() => void load()}
      />
    );
      ...
      <PageHeader
        ...
        meta={
          <span className="inline-flex items-center gap-1.5 text-xs text-muted">
            ...
            {days === null
              ? `Configurează data pentru ${weddingTitle}`
              : `${formatNumber(days)} zile până la ${weddingTitle}`}
          </span>
        }
      />
        ...
        <Card>
          <CardHeader>
            <div>
              ...
              <CardDescription>
                Un punct de pornire pentru o petrecere de 8–10 ore.
              </CardDescription>
            </div>
          ...
          <CardContent>
            <Field
              label="Număr de invitați"
              htmlFor="tool-guests"
              hint="Ajustează după confirmările RSVP și pachetul locației."
            >
              <Input
                id="tool-guests"
                type="number"
            ...
            <dl className="mt-5 grid grid-cols-2 gap-x-4 gap-y-5 border-y border-line py-5 sm:grid-cols-4">
              ...
              <ToolResult
                ...
                label="sticle spumant"
                note="6–7 pahare"
              />
              <ToolResult
                value={softDrinkLitres}
                label="litri răcoritoare"
                note="mix variat"
              />
            </dl>
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="max-w-xl text-xs leading-relaxed text-faint">
                Estimarea nu include cafea, cocktailuri sau băuturile deja
                incluse în meniul locației.
              </p>
        ...
        <Card>
          <CardHeader>
            <div>
              ...
              <CardDescription>
                Separă neprevăzutele înainte de alocarea bugetului.
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <Field label="Buget total" htmlFor="tool-budget">
                <Input
                  ...
                  value={budget}
                  onChange={(event) =>
                    setBudget(event.target.valueAsNumber || 0)
                  }
                />
              </Field>
              <Field label="Rezervă (%)" htmlFor="tool-reserve">
                <Input
                  ...
                  onChange={(event) =>
                    setReservePercent(event.target.valueAsNumber || 0)
                  }
                />
              </Field>
            </div>
            ...
            <div className="rounded-xl bg-subtle p-4">
              <div className="flex items-end justify-between gap-4">
                ...
                <Badge variant={safeReserve >= 10 ? "success" : "warning"}>
                  {safeReserve}%
                </Badge>
              </div>
              <Progress
                value={safeReserve}
                max={30}
                tone={safeReserve >= 10 ? "success" : "warning"}
                className="mt-4"
                aria-label="Procent rezervă"
              />
            </div>

            <div className="flex items-center justify-between border-t border-line pt-4">
              <span className="text-sm text-muted">
                Disponibil pentru planificare
              </span>
            ...
            <p className="flex items-start gap-2 text-xs leading-relaxed text-faint">
              <Check
                ...
                aria-hidden
              />
              Recomandarea uzuală este 10–15%; ajustați după câte contracte au
              deja preț fix.
            </p>
          </CardContent>
        </Card>
