...
export default function WeddingDayPage() {
      ...
      try {
        ...
        if (center.plan) {
          const [run, incidentList, checklistList] = await Promise.all([
            weddingOsApi.weddingDayRunOfShow(
              currentWorkspace.id,
              center.plan.id,
            ),
            weddingOsApi.weddingDayIncidents(
              currentWorkspace.id,
              center.plan.id,
            ),
            weddingOsApi.weddingDayChecklists(
              currentWorkspace.id,
              center.plan.id,
            ),
          ]);
  ...
  React.useEffect(() => {
    ...
    const stream = new EventSource(
      `/api/v1/workspaces/${encodeURIComponent(currentWorkspace.id)}/wedding-day/live`,
  ...
  const transitionPlan = async (
    ...
    setWorking(true);
    try {
      await weddingOsApi.transitionWeddingDayPlan(
        currentWorkspace.id,
        data.plan.id,
        data.plan.version,
        action,
                              ...
                              incident.status === "OPEN"
                                ...
                                : incident.status === "ACKNOWLEDGED"
                                  ...
                                  : incident.status === "INVESTIGATING"
                                    ? "MITIGATE"
                                    : "RESOLVE";
                            await weddingOsApi.transitionWeddingDayIncident(
                              currentWorkspace.id,
                              incident.id,
                              incident.version,
                              transition,
