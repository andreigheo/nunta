...
export default function ReviewsPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-5">
    <PageHeader title="Recenzii verificate" description="Poți evalua numai colaborări finalizate dintr-un booking Sarbato real." actions={items.some((item) => !item.review) ? <Button size="sm" onClick={() => void open(items.find((item) => !item.review)!)}><Pencil className="size-3.5" />Scrie o recenzie</Button> : undefined} />
    <div className="grid gap-4 lg:grid-cols-[1.35fr_0.65fr]">
      <Card className="border-brand-panel bg-brand-panel text-on-brand-panel"><CardContent className="p-5"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-on-brand-panel/65">Feedback verificabil</p><h2 className="mt-2 font-brand text-2xl font-semibold">{published} recenzii publicate din {items.length} colaborări eligibile</h2><p className="mt-2 text-sm text-on-brand-panel/75">Publicarea confirmă că experiența descrisă este autentică și legată de booking.</p></CardContent></Card>
