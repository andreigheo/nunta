...
export default function BookingsPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-4"><PageHeader title="Bookings" description="Rezervări create numai prin acceptarea explicită a unei oferte." />{items.length === 0 ? <EmptyState icon={CalendarClock} title="Niciun booking" description="Acceptă o ofertă pentru a crea atomic booking-ul, contractul și poziția de buget." /> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{items.map((item) => <Card key={item.id} interactive onClick={() => { setSelected(item); setDates({ start: item.serviceStartAt ? String(item.serviceStartAt).slice(0, 16) : "", end: item.serviceEndAt ? String(item.serviceEndAt).slice(0, 16) : "" }); }}><CardContent className="p-4"><div className="flex justify-between gap-3"><p className="font-semibold text-ink">{String(item.title)}</p><Badge variant={item.status === "COMPLETED" ? "success" : item.status === "CANCELLED" ? "danger" : "brand"} dot>{label(String(item.status))}</Badge></div><p className="mt-3 text-lg font-semibold text-ink">{formatRON(Number(item.totalMinor ?? 0) / 100)}</p><p className="text-xs text-muted">Plătit: {formatRON(Number(item.paidTotalMinor ?? 0) / 100)} · restant: {formatRON(Number(item.outstandingTotalMinor ?? 0) / 100)}</p><p className="mt-2 text-xs text-faint">{Array.isArray(item.items) ? item.items.length : 0} servicii · {Array.isArray(item.milestones) ? item.milestones.length : 0} milestone-uri</p></CardContent></Card>)}</div>}
