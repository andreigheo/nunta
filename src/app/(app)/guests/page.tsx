...
export default function GuestsPage() {
  ...
  const [preferredHouseholdId, setPreferredHouseholdId] = React.useState("");
  ...
  const focusedGuestOpenedRef = React.useRef(false);
  ...
  const bulkActions = React.useMemo<BulkAction[]>(() => [
    ...
    { value: "REMOVE_TAG", label: "Elimină eticheta", allowed: canWrite },
    { value: "MOVE_TO_HOUSEHOLD", label: "Mută în alt grup de invitați", allowed: canWrite },
  ...
  const load = React.useCallback(async (search: string, cursor?: string, append = false) => {
    ...
    try {
      ...
      if (!append && !focusedGuestOpenedRef.current) {
        const focusedGuestId = new URLSearchParams(window.location.search).get("focus");
        if (focusedGuestId) {
          focusedGuestOpenedRef.current = true;
          void weddingOsApi
            .guest(currentWorkspace.id, focusedGuestId)
            .then(setSelected)
            .catch(() => undefined);
        }
      }
  ...
  const createHousehold = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const created = await weddingOsApi.createHousehold(currentWorkspace.id, {
      ...
      setPreferredHouseholdId(created.id);
      toast({
        title: "Grupul a fost creat",
        description: "Acum adaugă prima persoană din acest grup.",
        variant: "success",
      });
      await load("");
      setGuestOpen(true);
    } catch (caught) { toast({ title: "Grupul nu a fost creat", description: apiErrorMessage(caught), variant: "error" }); }
  ...
  const deleteTag = async () => {
    ...
    try {
      ...
      toast({ title: "Etichetă ștearsă", description: result.affectedGuests ? `Eticheta a fost eliminată de la ${result.affectedGuests} invitați.` : undefined, variant: "success" });
      await load(query);
  ...
  const updateHousehold = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSelectedHousehold(null); toast({ title: "Grup actualizat", variant: "success" }); await load(query);
    } catch (caught) { toast({ title: "Actualizarea grupului a eșuat", description: apiErrorMessage(caught), variant: "error" }); }
    finally { setSaving(false); }
  };
  ...
  const archiveHousehold = async () => {
    ...
    try {
      ...
      setSelectedHousehold(null); toast({ title: "Grup arhivat", variant: "success" }); await load(query);
    } catch (caught) { toast({ title: "Arhivarea grupului a eșuat", description: apiErrorMessage(caught), variant: "error" }); }
    finally { setSaving(false); }
  };
  ...
  const exportGuests = async () => {
    ...
    try {
      ...
      await waitForJob(job.id); const blob = await weddingOsApi.downloadJobArtifact(job.id); downloadBlob(blob, "sarbato-invitati.xlsx");
  ...
  return <div className="mx-auto max-w-7xl space-y-5">
    <PageHeader title="Invitați" description="Adaugă persoanele, organizează cine primește aceeași invitație și urmărește confirmările într-un singur loc." actions={<>
      ...
      <Button variant="secondary" size="sm" disabled={!canWrite || demoMode} onClick={() => setHouseholdOpen(true)}><UsersRound className="size-3.5" />Grup nou</Button>
      <Button size="sm" disabled={!canWrite || demoMode} onClick={() => households.length ? setGuestOpen(true) : setHouseholdOpen(true)}><UserPlus className="size-4" />Adaugă persoane</Button>
    </>} />
    ...
    <Card>
      <CardContent className="p-4 sm:p-5">
        ...
        <ol className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
          <WorkflowStep number={1} title="Adaugă persoanele" value={`${summary.totalGuests} persoane · ${summary.totalHouseholds} grupuri`} complete={summary.totalGuests > 0} />
          <WorkflowStep number={2} title="Pregătește invitațiile" value={`${preparedInvitations} din ${summary.totalHouseholds} grupuri pregătite`} complete={summary.totalHouseholds > 0 && preparedInvitations >= summary.totalHouseholds} />
    ...
    {householdsTruncated ? <div className="rounded-lg bg-warning-soft px-4 py-3 text-sm text-warning" role="status">Sunt afișate primele 1.000 de grupuri în selectoare. Lista de invitați rămâne disponibilă, dar operațiile de mutare trebuie împărțite în loturi controlate.</div> : null}

    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 xl:grid-cols-8">
      <StatCard label="Total" value={summary.totalGuests} /><StatCard label="Grupuri" value={summary.totalHouseholds} />
    ...
    <Card><CardContent className="flex flex-wrap items-end gap-3 p-4">
      <Field label="Căutare" className="min-w-64 flex-1"><Input icon={<Search className="size-4" />} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Nume, grup, email sau telefon…" /></Field>
    ...
    {canSelectGuests && selectedIds.size > 0 && <Card><CardContent className="flex flex-wrap items-end gap-3 p-4">
      ...
      {effectiveBulkCommand === "MOVE_TO_HOUSEHOLD" && <Field label="Grup de invitați"><Select value={bulkTarget} onChange={(event) => setBulkTarget(event.target.value)}><option value="">Alege</option>{households.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></Field>}
    ...
    {error ? <Card><CardContent className="p-6"><p className="text-sm text-danger">{error}</p><Button className="mt-3" size="sm" onClick={() => void load(query)}>Reîncearcă</Button></CardContent></Card> :
      ...
      guests.length === 0 ? <EmptyState icon={Users} title="Lista de invitați este goală" description="Începe cu un grup: o persoană, o familie sau colegii care vor primi aceeași invitație." action={canWrite && !demoMode ? { label: "Adaugă primul grup", onClick: () => setHouseholdOpen(true) } : undefined} /> :
      <><div className="hidden md:block"><Table minWidth="1040px"><THead><TR>{canSelectGuests && <TH><Checkbox aria-label="Selectează pagina" checked={guests.length > 0 && guests.every((guest) => selectedIds.has(guest.id))} onCheckedChange={(checked) => setSelectedIds(checked ? new Set(guests.map((guest) => guest.id)) : new Set())} /></TH>}<TH>Invitat</TH><TH>Grup / familie</TH><TH>Contact</TH><TH>Partea</TH><TH>Invitație</TH><TH>RSVP / Meniu</TH><TH>Logistică</TH></TR></THead><TBody>
        {guests.map((guest) => <TR key={guest.id} onClick={() => void openGuest(guest)}>
          ...
          <TD className="text-muted"><button type="button" className="text-left hover:text-brand hover:underline" onClick={(event) => { event.stopPropagation(); setSelectedHousehold(households.find((item) => item.id === guest.householdId) ?? null); }}>{guest.householdName ?? households.find((item) => item.id === guest.householdId)?.name ?? "Fără grup"}</button></TD>
    ...
    <Modal open={guestOpen} onClose={() => { setGuestOpen(false); setGuestKind("adult"); setPrimaryGuestId(""); }} title="Invitat nou" description="Adaugă un adult, un copil sau persoana plus-unu aprobată pentru un invitat.">
      <form className="grid gap-4 sm:grid-cols-2" onSubmit={createGuest}>
        ...
        <Field label="Email"><Input name="email" type="email" autoComplete="email" /></Field><Field label="Telefon" hint="Include prefixul internațional, de exemplu +40. Același număr este folosit și pentru WhatsApp."><Input name="phone" type="tel" autoComplete="tel" inputMode="tel" placeholder="+40 7…" /></Field>
        {tags.length > 0 && <fieldset className="grid gap-2 rounded-xl border border-line p-3 sm:col-span-2"><legend className="px-1 text-sm font-medium text-ink">Etichete</legend><p className="text-xs leading-relaxed text-muted">Opțional. Folosește etichete precum „Prieteni” sau „Colegii mei” pentru filtre și mesaje rapide.</p><div className="flex flex-wrap gap-2">{tags.map((tag) => <label key={tag.id} className="flex cursor-pointer items-center gap-2 rounded-lg border border-line bg-surface px-3 py-2 text-sm text-ink transition-colors hover:border-brand/40"><input name="tagIds" value={tag.id} type="checkbox" className="size-4 accent-brand" /><span className="size-2 rounded-full" style={{ backgroundColor: tag.color ?? "var(--color-brand)" }} aria-hidden="true" />{tag.name}</label>)}</div></fieldset>}
    ...
    <Drawer open={Boolean(selected)} onClose={() => setSelected(null)} title={selected ? `${selected.firstName} ${selected.lastName}` : "Invitat"} width="xl">{selected && <Tabs defaultValue="profile" className="space-y-5 p-5"><TabsList><TabsTrigger value="profile">Profil</TabsTrigger><TabsTrigger value="participation">Participare</TabsTrigger><TabsTrigger value="organization">Organizare</TabsTrigger><TabsTrigger value="communication">Comunicare</TabsTrigger><TabsTrigger value="history">Istoric</TabsTrigger></TabsList>
      <TabsContent value="profile"><form key={`${selected.id}:${selected.version}`} className="space-y-4" onSubmit={updateGuest}><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><Field label="Prenume"><Input name="firstName" defaultValue={selected.firstName} required /></Field><Field label="Nume"><Input name="lastName" defaultValue={selected.lastName} required /></Field></div><Field label="Nume afișat" hint="Cum vrei să fie adresată persoana în invitație."><Input name="displayName" defaultValue={selected.displayName ?? ""} /></Field><Field label="Grup / familie" hint="Persoanele din același grup primesc aceeași invitație."><Select name="householdId" defaultValue={selected.householdId}>{households.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></Field><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><Field label="Email"><Input name="email" type="email" defaultValue={selected.email ?? ""} /></Field><Field label="Telefon" hint="Cu prefix internațional."><Input name="phone" type="tel" inputMode="tel" defaultValue={selected.phone ?? ""} /></Field></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><Field label="Relație"><Input name="relationship" defaultValue={selected.relationship ?? ""} /></Field><Field label="Categorie"><Input name="category" defaultValue={selected.category ?? ""} /></Field></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><Field label="Limba comunicării"><Select name="preferredLanguage" defaultValue={selected.preferredLanguage}><LanguageOptions /></Select></Field><Field label="Parte"><Select name="side" defaultValue={sideInput(selected.side)}><SideOptions /></Select></Field></div><div className="flex flex-wrap gap-2">{selected.tags.map((tag) => <Badge key={tag.id} variant="neutral">{tag.name}</Badge>)}</div><label className="flex items-center gap-2 text-sm"><input name="plusOneAllowed" type="checkbox" defaultChecked={selected.plusOneAllowed} className="size-4 accent-brand" />Permite plus-unu</label><div className="flex flex-wrap gap-2">{canWrite && <Button type="submit" loading={saving} disabled={saving}>Salvează profilul</Button>}{canArchive && <Button type="button" variant="destructive" disabled={saving} onClick={() => void archiveGuest()}><Archive className="size-4" />Arhivează</Button>}</div></form></TabsContent>
      <TabsContent value="participation"><div className="grid gap-3"><DetailPanel title="Confirmare și invitație" lines={[selected.rsvpStatus ? `RSVP: ${rsvpLabel(selected.rsvpStatus)}` : "RSVP: fără răspuns", `Invitație: ${invitationLabel[selected.invitationStatus ?? ""] ?? "Nepregătită"}`]} /><DetailPanel title="Participare la evenimente" lines={["Răspunsurile detaliate sunt administrate în pagina RSVP.", selected.rsvpStatus ? `Status general: ${rsvpLabel(selected.rsvpStatus)}` : "Nu există încă un răspuns publicat."]} /><DetailPanel title="Meniu" lines={[selected.menuName ?? "Meniul nu a fost selectat încă.", "Alergiile și excepțiile sunt disponibile în pagina Meniuri."]} /><Button variant="outline" onClick={() => router.push("/rsvp")}>Deschide toate răspunsurile RSVP</Button></div></TabsContent>
      <TabsContent value="organization"><div className="space-y-4"><form key={`logistics:${selected.id}:${selected.version}`} className="space-y-3 rounded-xl border border-line p-4" onSubmit={updateGuest}><div><h3 className="font-medium text-ink">Nevoi logistice</h3><p className="mt-1 text-sm text-muted">Bifează ce trebuie pregătit pentru această persoană.</p></div><input type="hidden" name="formScope" value="logistics" /><label className="flex min-h-11 items-center gap-2 text-sm"><input name="needsTransport" type="checkbox" defaultChecked={selected.needsTransport} className="size-4 accent-brand" />Are nevoie de transport</label><label className="flex min-h-11 items-center gap-2 text-sm"><input name="needsAccommodation" type="checkbox" defaultChecked={selected.needsAccommodation} className="size-4 accent-brand" />Are nevoie de cazare</label>{canWrite && <Button type="submit" disabled={saving}>Salvează nevoile</Button>}</form><div className="grid gap-3 sm:grid-cols-3"><GuestDestinationCard title="Loc la masă" status={selected.rsvpStatus === "confirmed" ? "Poate fi așezat" : "Necesită RSVP confirmat"} action="Așază la masă" onClick={() => router.push(`/seating?guest=${selected.id}&returnTo=${encodeURIComponent(`/guests?focus=${selected.id}`)}`)} /><GuestDestinationCard title="Transport" status={selected.needsTransport ? "Transport solicitat" : "Nicio solicitare"} action="Organizează transportul" onClick={() => router.push(`/transport?guest=${selected.id}&returnTo=${encodeURIComponent(`/guests?focus=${selected.id}`)}`)} /><GuestDestinationCard title="Cazare" status={selected.needsAccommodation ? "Cazare solicitată" : "Nicio solicitare"} action="Organizează cazarea" onClick={() => router.push(`/accommodation?guest=${selected.id}&returnTo=${encodeURIComponent(`/guests?focus=${selected.id}`)}`)} /></div></div></TabsContent>
      <TabsContent value="communication"><div className="space-y-4"><div className="space-y-3">{selected.communication?.length ? selected.communication.map((item) => <div key={item.id} className="rounded-xl border border-line p-3"><div className="flex justify-between gap-3 text-xs text-faint"><span>{item.channel} · {item.direction}</span><span>{new Date(item.occurredAt).toLocaleString("ro-RO")}</span></div><p className="mt-1 text-sm">{item.summary}</p></div>) : <p className="rounded-xl bg-subtle p-4 text-sm text-muted">Nu există comunicări înregistrate pentru acest invitat.</p>}</div><form className="space-y-3 rounded-xl border border-line p-4" onSubmit={savePrivateNote}><Field label="Notă privată" hint="Conținutul este criptat și nu este reafișat după salvare."><Textarea name="notesPrivate" rows={5} maxLength={4000} required /></Field>{canWrite && <Button type="submit" disabled={saving}>Salvează nota</Button>}</form></div></TabsContent>
      <TabsContent value="history"><DetailPanel title="Istoricul persoanei" lines={["Crearea, actualizările, răspunsurile și comunicările sunt păstrate în jurnalul de activitate."]} /><Button className="mt-3" variant="outline" onClick={() => router.push("/activity")}>Deschide istoricul complet</Button></TabsContent>
    </Tabs>}</Drawer>
  </div>;
}
...
function GuestDestinationCard({ title, status, action, onClick }: { title: string; status: string; action: string; onClick: () => void }) {
  return <div className="flex min-h-40 flex-col rounded-xl border border-line bg-subtle p-4"><h3 className="font-semibold text-ink">{title}</h3><p className="mt-1 text-sm leading-5 text-muted">{status}</p><Button className="mt-auto w-full" variant="outline" onClick={onClick}>{action}<ArrowRight className="size-4" aria-hidden /></Button></div>;
}
