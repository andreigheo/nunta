...
export default function GuestsPage() {
  ...
  const bulkActions = React.useMemo<BulkAction[]>(() => [
    { value: "ADD_TAG", label: "Adaugă în grup", allowed: canWrite },
    { value: "REMOVE_TAG", label: "Elimină din grup", allowed: canWrite },
  ...
  const createTag = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setTagOpen(false); setEditingTag(null); toast({ title: editingTag ? "Grup actualizat" : "Grup creat", variant: "success" }); await load(query);
    } catch (caught) { toast({ title: editingTag ? "Grupul nu a fost actualizat" : "Grupul nu a fost creat", description: apiErrorMessage(caught), variant: "error" }); }
    finally { setSaving(false); }
  };
  ...
  const deleteTag = async () => {
    ...
    try {
      ...
      toast({ title: "Grup șters", description: result.affectedGuests ? `${result.affectedGuests} invitați au fost eliminați din grup.` : undefined, variant: "success" });
      await load(query);
    } catch (caught) { toast({ title: "Grupul nu a fost șters", description: apiErrorMessage(caught), variant: "error" }); }
    finally { setSaving(false); }
  };
  ...
  return <div className="mx-auto max-w-7xl space-y-5">
    ...
    <Card><CardContent className="flex flex-wrap items-end gap-3 p-4">
      ...
      <Field label="Grup"><Select value={tagFilter} onChange={(event) => setTagFilter(event.target.value)}><option value="">Toate</option>{tags.map((tag) => <option key={tag.id} value={tag.id}>{tag.name}</option>)}</Select></Field>
      ...
      <Button size="sm" variant="outline" disabled={!canWrite || demoMode} onClick={() => { setEditingTag(null); setTagOpen(true); }}><Tag className="size-3.5" />Gestionează grupuri</Button>
    </CardContent></Card>
    {canSelectGuests && selectedIds.size > 0 && <Card><CardContent className="flex flex-wrap items-end gap-3 p-4">
      ...
      {["ADD_TAG", "REMOVE_TAG"].includes(effectiveBulkCommand) && <Field label="Grup"><Select value={bulkTarget} onChange={(event) => setBulkTarget(event.target.value)}><option value="">Alege</option>{tags.map((tag) => <option key={tag.id} value={tag.id}>{tag.name}</option>)}</Select></Field>}
    ...
    <Modal open={guestOpen} onClose={() => { setGuestOpen(false); setGuestKind("adult"); setPrimaryGuestId(""); }} title="Invitat nou" description="Adaugă un adult, un copil sau persoana plus-unu aprobată pentru un invitat.">
      <form className="grid gap-4 sm:grid-cols-2" onSubmit={createGuest}>
        ...
        {tags.length > 0 && <fieldset className="grid gap-2 rounded-xl border border-line p-3 sm:col-span-2"><legend className="px-1 text-sm font-medium text-ink">Grupuri</legend><p className="text-xs leading-relaxed text-muted">Opțional. Poți trimite ulterior un singur mesaj tuturor invitaților dintr-un grup.</p><div className="flex flex-wrap gap-2">{tags.map((tag) => <label key={tag.id} className="flex cursor-pointer items-center gap-2 rounded-lg border border-line bg-surface px-3 py-2 text-sm text-ink transition-colors hover:border-brand/40"><input name="tagIds" value={tag.id} type="checkbox" className="size-4 accent-brand" /><span className="size-2 rounded-full" style={{ backgroundColor: tag.color ?? "var(--color-brand)" }} aria-hidden="true" />{tag.name}</label>)}</div></fieldset>}
    ...
    <Modal open={tagOpen} onClose={() => { setTagOpen(false); setEditingTag(null); }} title={editingTag ? "Editează grupul" : "Grupuri de invitați"} description="Creează un grup, apoi adaugă invitații în el pentru selecții și trimiteri rapide."><div className="space-y-5">{!editingTag && tags.length ? <div className="space-y-2">{tags.map((tag) => <div key={tag.id} className="flex items-center justify-between gap-3 rounded-lg border border-line p-3"><span className="flex min-w-0 items-center gap-2"><span className="size-3 shrink-0 rounded-full" style={{ backgroundColor: tag.color ?? "#6d5dfc" }} /><span className="truncate text-sm font-medium text-ink">{tag.name}</span><span className="text-xs text-muted">{tag.assignedGuests ?? 0} invitați</span></span><span className="flex gap-1"><Button type="button" size="icon-sm" variant="ghost" aria-label={`Editează grupul ${tag.name}`} onClick={() => setEditingTag(tag)}><Pencil className="size-4" /></Button><Button type="button" size="icon-sm" variant="ghost" aria-label={`Șterge grupul ${tag.name}`} onClick={() => setTagToDelete(tag)}><Trash2 className="size-4 text-danger" /></Button></span></div>)}</div> : null}<form key={editingTag?.id ?? "new"} className="space-y-4" onSubmit={createTag}><Field label={editingTag ? "Nume" : "Grup nou"} required><Input name="name" defaultValue={editingTag?.name ?? ""} required /></Field><Field label="Culoare" hint="Format hex, de exemplu #6d5dfc"><Input name="color" defaultValue={editingTag?.color ?? "#6d5dfc"} pattern="#[0-9a-fA-F]{6}" /></Field><div className="flex justify-end gap-2">{editingTag ? <Button type="button" variant="ghost" onClick={() => setEditingTag(null)}>Înapoi la listă</Button> : <Button type="button" variant="ghost" onClick={() => setTagOpen(false)}>Închide</Button>}<Button type="submit" disabled={saving}>{editingTag ? "Salvează" : "Creează grupul"}</Button></div></form></div></Modal>
    <ConfirmDialog open={Boolean(tagToDelete)} onClose={() => setTagToDelete(null)} onConfirm={() => void deleteTag()} title="Ștergi grupul?" description="Grupul va fi eliminat, dar invitații și datele lor rămân în listă." confirmLabel="Șterge grupul" destructive loading={saving} />
